...
const f8 = () => {
    a8_res = a8.map(item => {
        return item.reduce((accum, item) => accum + item);
    })
    console.log(a8);
    console.log(a8_res);
}
...
const f9 = () => {
    a9.map((item, index) => {
        if (item % 2 === 0) {
            a9_index.push(index);
            return a9_even.push(item);
        }
        return a9_index.push(index);
    })
    console.log(a9);
